...
async function getLivePixToken({ clientId, clientSecret }) {
  ...
  const { data } = await axios.post("https://oauth.livepix.gg/oauth2/token", body.toString(), {
    headers: { "Content-Type": "application/x-www-form-urlencoded" }
  });
  return data.access_token;
}
async function createPayment({ token, amountCents, currency = "BRL", redirectUrl }) {
  const { data } = await axios.post("https://api.livepix.gg/v2/payments", { amount: amountCents, currency, redirectUrl }, {
    headers: { Authorization: `Bearer ${token}` }
  });
  return data.data;
}
async function registerWebhook({ token, url }) {
  const { data } = await axios.post("https://api.livepix.gg/v2/webhooks", { url }, {
    headers: { Authorization: `Bearer ${token}` }
  });
  return data.data;
}
